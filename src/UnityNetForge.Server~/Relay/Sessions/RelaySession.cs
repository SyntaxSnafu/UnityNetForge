...
namespace UnityNetForge.Relay.Server.Relay.Sessions;
...
public class RelaySession
{
    ...
    {
        if (message.AuthorClientId == 0 && host.HasValue)
        {
            message.AuthorClientId = (ulong)host.Value;
        }
        ...
        switch (message.Type)
        {
            ...
            case RelayControlMessageType.KickFromRelay:
                ...
                if (from.Id == host && host != (int)target)
                {
                    if (_connections.TryGetValue((int)target, out var targetPeer))
                    {
                        _connections.Remove((int)target);
                        await _server.UnityNetForgeManager.DisconnectPeerAsync(targetPeer);
    ...
    public async Task DisconnectAll()
    {
        ...
        {
            await this._server.UnityNetForgeManager.DisconnectPeerAsync(con);
        }
        _connections.Clear();
    }
