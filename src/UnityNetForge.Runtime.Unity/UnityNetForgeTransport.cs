...
using Unity.Netcode;
using UnityEngine;
namespace UnityNetForge.Runtime.Unity
{
    public class UnityNetForgeTransport : NetworkTransport
    {
        enum HostType
        {
            None,
        ...
        UnityNetForgeManager _mUnityNetForgeManager;
        ...
        async ValueTask OnNetworkReceive(PeerBase peer, CompositeReader reader, byte channel, DeliveryMethod deliveryMethod)
        {
            ...
            switch (message.Type)
            {
                ...
                case RelayControlMessageType.KickFromRelay:
                    break;
                default:
                    Debug.LogError("Ignoring UnityNetForge control byte " + message.Type);
                    break;
            }
        }
        ...
        bool ConnectToRelayServer(string joinCode)
        {
            _ = Task.Run(async () =>
            {
                _mUnityNetForgeManager.Bind(IPAddress.Any, IPAddress.None, 0);
                var ms = CompositeWriter.Create();
                ms.Write(joinCode);
                _relayPeer = await _mUnityNetForgeManager.ConnectAsync(NetUtils.MakeEndPoint(Address, Port), ms);
                await _mUnityNetForgeManager.ListenAsync(_cts.Token);
            }, _cts.Token);
            return true;
        }
        ...
        {
            _ = Task.Run(() => _mUnityNetForgeManager.DisconnectAllPeersAsync());
            _relayPeer = null;
        }
        ...
        {
            Debug.Log("Shutdown");
            _mUnityNetForgeManager.OnConnectionRequest -= OnConnectionRequest;
            _mUnityNetForgeManager.OnPeerDisconnected -= OnPeerDisconnected;
            _mUnityNetForgeManager.OnReceive -= OnNetworkReceive;
            _ = _mUnityNetForgeManager.StopAsync();
        ...
        {
            _mUnityNetForgeManager = new UnityNetForgeManager
            {
