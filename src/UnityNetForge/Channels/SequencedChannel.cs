...
namespace UnityNetForge
{
    internal sealed class SequencedChannel : ChannelBase
    {
        ...
        public override async ValueTask<bool> HandlePacketAsync(NetworkPacket packet)
        {
            ...
            {
                if (Peer.UnityNetForgeManager.RecordNetworkStatistics)
                {
                    Peer.Statistics.AddPacketLoss(relative - 1);
                    Peer.UnityNetForgeManager.Statistics.AddPacketLoss(relative - 1);
                }

                _remoteSequence = packet.Sequence;
                await Peer.UnityNetForgeManager.CreateReceiveEvent(
                    packet,
