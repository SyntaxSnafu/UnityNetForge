...
{
    /// <summary>
    ///     Represents a remote peer, managed by a given <see cref="UnityNetForgeManager" /> instance.
    /// </summary>
    public partial class PeerBase
    {
        ...
        /// <summary>
        ///     The <see cref="UnityNetForgeManager" /> instance responsible for this peer.
        /// </summary>
        public readonly UnityNetForgeManager UnityNetForgeManager;
        ...
        protected PeerBase(UnityNetForgeManager unityNetForgeManager, IPEndPoint remote, int id,
            long connectTime, byte connectionNumber)
        {
            Id = id;
            Statistics = new NetStatistics();
            UnityNetForgeManager = unityNetForgeManager;
            ResetMtu();
            EndPoint = remote;
            ...
            _channels = new ChannelBase[UnityNetForgeManager.ChannelsCount * NetConstants.ChannelTypeCount];
        ...
        /// <summary>
        ///     The current delta between the remote peer's time and the <see cref="UnityNetForgeManager" />'s local time.
        ...
        internal void ResetMtu()
        {
            ...
            if (UnityNetForgeManager.MtuOverride > 0)
                OverrideMtu(UnityNetForgeManager.MtuOverride);
            else if (UnityNetForgeManager.UseSafeMtu)
                SetMtu(0);
            else
                SetMtu(1);
        }
        ...
        private void SetMtu(int mtuIdx)
        {
            ...
            MaximumTransferUnit = NetConstants.PossibleMtu[mtuIdx] - UnityNetForgeManager.ExtraPacketSizeForLayer;
        }
        ...
        internal static async Task<OutgoingPeer> ConnectToAsync(UnityNetForgeManager manager, IPEndPoint remote, int id,
        ...
        internal static async Task<IncomingPeer> AcceptAsync(UnityNetForgeManager unityNetForgeManager, ConnectionRequest request,
            int id)
        {
            var peer = new IncomingPeer(unityNetForgeManager, request.RemoteEndPoint, id,
 ...
 // That means we must send {totalPackets} total packets.");
        ...
        internal async Task<ShutdownResult> ShutdownAsync(ArraySegment<byte> data, bool force)
        {
            ...
            try
            {
                ...
                await UnityNetForgeManager.RawSendAsync(_shutdownPacket.Value, EndPoint);
                return result;
            }
            finally
            {
        ...
        internal async ValueTask AddReliablePacket(DeliveryMethod method, NetworkPacket p)
        {
            if (p.IsFragmented)
            {
                ...
                await UnityNetForgeManager.CreateReceiveEvent(resultingPacket, method,
                    (byte)(packetChannelId / NetConstants.ChannelTypeCount), 0, this);
            }
            else //Just simple packet
            {
                await UnityNetForgeManager.CreateReceiveEvent(p, method, (byte)(p.ChannelId / NetConstants.ChannelTypeCount),
            ...
            switch (packet.Property)
            {
                ...
                case PacketProperty.Ping:
                    if (NetUtils.RelativeSequenceNumber(packet.Sequence, _pongPacket.Sequence) > 0)
                    {
                        ...
                        await UnityNetForgeManager.RawSendAsync(_pongPacket, EndPoint);
                    }
                ...
                case PacketProperty.Pong:
                    if (packet.Sequence == _pingPacket.Sequence)
                    {
                        ...
                        await UnityNetForgeManager.ConnectionLatencyUpdated(this, elapsedMs / 2);
                    }
                ...
                case PacketProperty.Unreliable:
                    await UnityNetForgeManager.CreateReceiveEvent(packet, DeliveryMethod.Unreliable, 0,
        ...
        private async Task SendMerged()
        {
            ...
            if (_mergeCount > 1)
            {
                ...
                bytesSent = await UnityNetForgeManager.RawSendAsync(
            ...
            if (UnityNetForgeManager.RecordNetworkStatistics)
            {
        ...
        internal async Task Update(long deltaTime)
        {
            ...
            switch (ConnectionState)
            {
                case ConnectionState.Connected:
                    if (_timeSinceLastPacket > UnityNetForgeManager.DisconnectTimeout)
                    {
                        LogDebug(
                            $"[UPDATE] Disconnect by timeout: {_timeSinceLastPacket} > {UnityNetForgeManager.DisconnectTimeout}");
                        await UnityNetForgeManager.ForceDisconnectPeerAsync(this, DisconnectReason.Timeout);
                        return;
                    }
                ...
                case ConnectionState.Outgoing when this is OutgoingPeer op:
                    ...
                    if (_connectTimer > UnityNetForgeManager.ReconnectDelay)
                    {
                        _connectTimer = 0;
                        _connectionAttempts++;
                        if (_connectionAttempts > UnityNetForgeManager.MaximumConnectionAttempts)
                        {
                            await UnityNetForgeManager.ForceDisconnectPeerAsync(this, DisconnectReason.ConnectionFailed);
                            return;
                        }
        ...
        internal async Task RecycleAndDeliver(NetworkPacket packet)
        {
            if (packet.IsFragmented)
            {
                ...
                {
                    // TODO FIX THIS
                    await UnityNetForgeManager.MessageDelivered(this, null);
